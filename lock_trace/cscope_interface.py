...
class FunctionAssignment:
    ...
    line: int
    context: str
    struct_name: Optional[str] = None  # The structure name if detected (e.g., rtnl_link_ops)
...
class CscopeInterface:
    ...
    def _validate_database(self) -> None:
        ...
        try:
            ...
            result = subprocess.run(
                ["cscope", "-d", "-f", str(self.cscope_file), "-L", "-0", "main"],
                cwd=self.source_dir,
                capture_output=True,
                text=True,
                timeout=10,
    ...
    def get_functions_called_by(self, function: str) -> List[FunctionCall]:
        ...
        try:
            result = subprocess.run(
                ["cscope", "-d", "-f", str(self.cscope_file), "-L", "-2", function],
                cwd=self.source_dir,
                capture_output=True,
                text=True,
                timeout=30,
            ...
            return self._parse_cscope_output(result.stdout)

        except subprocess.TimeoutExpired:
            raise RuntimeError(f"Cscope query for {function} timed out")

    def get_functions_calling(self, function: str) -> List[FunctionCall]:
        ...
        try:
            result = subprocess.run(
                ["cscope", "-d", "-f", str(self.cscope_file), "-L", "-3", function],
                cwd=self.source_dir,
                capture_output=True,
                text=True,
                timeout=30,
    ...
    def function_exists(self, function: str) -> bool:
        ...
        try:
            result = subprocess.run(
                ["cscope", "-d", "-f", str(self.cscope_file), "-L", "-1", function],
                cwd=self.source_dir,
                capture_output=True,
                text=True,
                timeout=10,
    ...
    def find_function_definition(self, function: str) -> Optional[FunctionCall]:
        ...
        try:
            ...
            calls = self._parse_cscope_output(result.stdout)
            return calls[0] if calls else None

        except subprocess.TimeoutExpired:
            return None

    def find_function_assignments(self, function: str) -> List[FunctionAssignment]:
        ...
        try:
            result = subprocess.run(
                ["cscope", "-d", "-f", str(self.cscope_file), "-L", "-0", function],
                cwd=self.source_dir,
                capture_output=True,
                text=True,
                timeout=30,
            ...
            return self._parse_function_assignments(result.stdout, function)

        except subprocess.TimeoutExpired:
            raise RuntimeError(f"Cscope assignment query for {function} timed out")

    def _parse_function_assignments(self, output: str, function: str) -> List[FunctionAssignment]:
    ...
    def get_callback_callers(self, function: str) -> List[FunctionCall]:
        ...
        direct_callers = self.get_functions_calling(function)
        ...
        # Look for callback assignments
        assignments = self.find_function_assignments(function)
        callback_callers = []

        for assignment in assignments:
            # Search for callers of the operation
            operation_callers = self.get_functions_calling(assignment.operation)

            # Filter callers by structure type context if possible
            filtered_callers = self._filter_callers_by_struct_context(
    ...
    def _filter_callers_by_struct_context(
        ...
        struct_name = self._extract_struct_name_from_assignment(assignment)
        if not struct_name:
        ...
        struct_info = self._get_struct_info(struct_name, assignment.file)
        if not struct_info:
            return callers
        ...
        for caller in callers:
            if self._is_caller_using_compatible_struct(caller, struct_name, struct_info):
                filtered_callers.append(caller)

        return filtered_callers

    def _extract_struct_name_from_assignment(self, assignment: FunctionAssignment) -> Optional[str]:
        ...
        try:
            ...
            end_line = min(len(lines), assignment.line + 10)  # Look up to 10 lines after
            ...
            for i in range(start_line, end_line):
                ...
                match = static_struct_pattern.search(line) or struct_pattern.search(line)
                if match:
    ...
    def _get_struct_info(self, struct_name: str, context_file: str) -> Optional[dict]:
        ...
        try:
            ...
            result = subprocess.run(
                ["cscope", "-d", "-f", str(self.cscope_file), "-L", "-1", f"struct {struct_name}"],
                cwd=self.source_dir,
                capture_output=True,
                text=True,
                timeout=10,
    ...
    def _is_caller_using_compatible_struct(
