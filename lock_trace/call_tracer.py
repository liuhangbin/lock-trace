...
class CallTracer:
    ...
    def __init__(self, cscope: CscopeInterface, max_depth: int = 10, enable_callback_search: bool = True):
    ...
    def _should_exclude_path(
        ...
        if exclude_directories:
            for func in path:
                ...
                func_def = self.cscope.find_function_definition(func)
    ...
    def build_call_graph(self, root_functions: List[str]) -> CallGraph:
        ...
        while queue:
            ...
            callees = self.cscope.get_functions_called_by(function)
            self._call_graph.callees[function] = callees

            # Get functions calling this function
            callers = self.cscope.get_functions_calling(function)
    ...
    def trace_callers(
        self, target_function: str, max_depth: Optional[int] = None, exclude_functions: Optional[Set[str]] = None, exclude_directories: Optional[Set[str]] = None
    ) -> List[CallPath]:
        ...
        def _trace_recursive(current_func: str, path: List[str], depth: int):
            ...
            if not self._should_exclude_path(reversed_path, exclude_functions, exclude_directories):
            ...
            if self.enable_callback_search:
                callers = self.cscope.get_callback_callers(current_func)
            else:
                callers = self.cscope.get_functions_calling(current_func)

            for caller in callers:
                _trace_recursive(caller.function, current_path, depth + 1)

            visited.remove(current_func)

        _trace_recursive(target_function, [], 0)
        return paths

    def trace_callees(
        self, source_function: str, max_depth: Optional[int] = None, exclude_functions: Optional[Set[str]] = None, exclude_directories: Optional[Set[str]] = None
    ) -> List[CallPath]:
        ...
        def _trace_recursive(current_func: str, path: List[str], depth: int):
            ...
            # Add current path if not excluded
            current_path = path + [current_func]
            if not self._should_exclude_path(current_path, exclude_functions, exclude_directories):
                paths.append(CallPath(functions=current_path, depth=depth))

            # Continue tracing callees
            visited.add(current_func)
            callees = self.cscope.get_functions_called_by(current_func)

            for callee in callees:
                _trace_recursive(callee.function, current_path, depth + 1)

            visited.remove(current_func)

        _trace_recursive(source_function, [], 0)
        return paths
    ...
    def find_call_paths(
        ...
        _find_paths_recursive(from_function, to_function, [], 0, set())
        return paths

    def get_function_depth_map(self, root_functions: List[str]) -> Dict[str, int]:
    ...
    def get_call_statistics(self, function: str) -> Dict[str, int]:
    ...
    def get_unique_call_chains(
        ...
        """
        all_paths = self.trace_callers(target_function, max_depth, exclude_functions, exclude_directories)
    ...
    def get_unique_callee_chains(
        ...
        """
        all_paths = self.trace_callees(source_function, max_depth, exclude_functions, exclude_directories)
