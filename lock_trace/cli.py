...
class LockTraceCLI:
    ...
    def trace_callers(
        ...
        """
        self._trace_function_paths(function, "callers", max_depth, tree, verbose, exclude_functions, exclude_directories)

    def _trace_function_paths(
        self,
        function: str,
        direction: str,
        max_depth: Optional[int] = None,
        ...
        Args:
            ...
            exclude_directories: Set of directory names to exclude from paths
        """
        if not self.cscope.function_exists(function):
            print(
        ...
        if direction == "callers":
            direction_text = "to"
            def get_all_paths(func, depth):
                return self.tracer.trace_callers(func, depth, exclude_functions, exclude_directories)
            def get_unique_paths(func, depth):
                return self.tracer.get_unique_call_chains(func, depth, exclude_functions, exclude_directories)
        else:  # callees
            direction_text = "from"
            def get_all_paths(func, depth):
                return self.tracer.trace_callees(func, depth, exclude_functions, exclude_directories)
            def get_unique_paths(func, depth):
                return self.tracer.get_unique_callee_chains(func, depth, exclude_functions, exclude_directories)

        if tree:
            # Use unique call chains for tree display
            paths = get_unique_paths(function, max_depth)
            ...
            paths = get_all_paths(function, max_depth)
    ...
    def trace_callees(
        ...
        Args:
            ...
            exclude_directories: Set of directory names to exclude from paths
        """
        self._trace_function_paths(function, "callees", max_depth, tree, verbose, exclude_functions, exclude_directories)

    def check_lock_protection(
        self, function: str, lock_name: str, tree: bool = False, verbose: bool = False, exclude_functions: Optional[Set[str]] = None, exclude_directories: Optional[Set[str]] = None
    ) -> None:
        ...
        results = self.analyzer.check_lock_protection(function, lock_name, unique_only, exclude_functions, exclude_directories)
    ...
    def analyze_lock_context(
        ...
        unique_only = not verbose
        contexts = self.analyzer.analyze_lock_context(function, locks, unique_only, exclude_functions, exclude_directories)
                ...
                if context.lock_operations:
                    ...
                    for op in context.lock_operations:
                        ...
                        if op.lock_type.value == "custom":
                            ...
                            elif "netlink" in op.lock_name.lower() or "genl" in op.lock_name.lower():
    ...
    def find_unprotected_calls(
        ...
        unique_only = not verbose
        unprotected = self.analyzer.find_unprotected_calls(
            function, required_locks, unique_only, exclude_functions, exclude_directories
        )
    ...
    def get_function_stats(self, function: str) -> None:
        ...
        # Get call statistics
        call_stats = self.tracer.get_call_statistics(function)

        # Get lock summary
        lock_summary = self.analyzer.get_lock_summary(function)
...
Examples:
  ...
  lock-trace stats my_function
    ...
    callers_parser.add_argument(
        "--max-depth", type=int, help="Override global max depth"
    )
    ...
    lock_context_parser.add_argument(
        "locks", nargs="?", help="Optional comma-separated list of specific locks to track"
    )
...
def main():
    ...
    try:
        if args.command == "callers":
            ...
            cli.trace_callers(args.function, max_depth, args.tree, args.verbose, exclude_functions, exclude_directories)
            ...
            cli.trace_callees(args.function, max_depth, args.tree, args.verbose, exclude_functions, exclude_directories)

        elif args.command == "lock-check":
            cli.check_lock_protection(args.function, args.lock, args.tree, args.verbose, exclude_functions, exclude_directories)

        elif args.command == "lock-context":
            locks = args.locks.split(",") if args.locks else None
            cli.analyze_lock_context(args.function, locks, args.tree, args.verbose, exclude_functions, exclude_directories)

        elif args.command == "unprotected":
            required_locks = args.required_locks.split(",")
            cli.find_unprotected_calls(
                args.function, required_locks, args.tree, args.verbose, exclude_functions, exclude_directories
            )

        elif args.command == "stats":
            cli.get_function_stats(args.function)
