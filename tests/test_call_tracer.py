...
class TestCallTracer:
    ...
    def setup_method(self):
        ...
        self.mock_cscope.get_callback_callers = Mock()
    ...
    def test_trace_callers_simple(self):
        ...
        paths = self.tracer.trace_callers("target_func")
    ...
    def test_trace_callers_max_depth(self):
        ...
        paths = self.tracer.trace_callers("target_func", max_depth=2)

        # Verify no path exceeds max depth
        for path in paths:
            assert path.depth <= 2

    def test_trace_callees_simple(self):
        ...
        paths = self.tracer.trace_callees("source_func")
    ...
    def test_find_call_paths(self):
        ...
        paths = self.tracer.find_call_paths("func_a", "target_func")
    ...
    def test_get_function_depth_map(self):
        ...
        depth_map = self.tracer.get_function_depth_map(["root_func"])
    ...
    def test_get_call_statistics(self):
        ...
        stats = self.tracer.get_call_statistics("test_func")
    ...
    def test_circular_dependency_handling(self):
        ...
        paths = self.tracer.trace_callees("func_a", max_depth=5)

        # Should complete without hanging
        assert isinstance(paths, list)

    def test_trace_callers_with_exclude_functions(self):
        """Test caller tracing with excluded functions."""
        # Mock cscope responses with sufficient return values
        def mock_get_callback_callers(function):
        ...
        paths = self.tracer.trace_callers("target_func", exclude_functions=exclude_functions)

        # Check that paths containing excluded_func are filtered out
        for path in paths:
            assert "excluded_func" not in path.functions

    def test_trace_callees_with_exclude_functions(self):
        """Test callee tracing with excluded functions."""
        # Mock cscope responses with sufficient return values
        def mock_get_functions_called_by(function):
        ...
        self.mock_cscope.get_functions_called_by.side_effect = mock_get_functions_called_by

        exclude_functions = {"excluded_func"}
        paths = self.tracer.trace_callees("source_func", exclude_functions=exclude_functions)

        # Check that paths containing excluded_func are filtered out
        for path in paths:
            assert "excluded_func" not in path.functions

    def test_get_unique_call_chains_with_exclude_functions(self):
        ...
        exclude_functions = {"excluded_func"}
        paths = self.tracer.get_unique_call_chains("target_func", exclude_functions=exclude_functions)

        # Check that paths containing excluded_func are filtered out
        for path in paths:
            assert "excluded_func" not in path.functions

    def test_get_unique_callee_chains_with_exclude_functions(self):
        ...
        exclude_functions = {"excluded_func"}
        paths = self.tracer.get_unique_callee_chains("source_func", exclude_functions=exclude_functions)
    ...
    def test_should_exclude_path(self):
        ...
        assert self.tracer._should_exclude_path(["func_x", "func_a", "func_y"], exclude_functions) is True
        assert self.tracer._should_exclude_path(["func_b", "func_x"], exclude_functions) is True

        # Path not containing excluded functions should not be excluded
        assert not self.tracer._should_exclude_path(["func_x", "func_y", "func_z"], exclude_functions)

        # Test with no excluded functions
        assert not self.tracer._should_exclude_path(["func_a", "func_b"], None)
        assert not self.tracer._should_exclude_path(["func_a", "func_b"], set())

    def test_should_exclude_path_with_directories(self):
        """Test _should_exclude_path method with directory exclusions."""
        # Mock function definitions for directory testing
        def mock_find_function_definition(function):
            file_map = {
                "func_a": FunctionCall("func_a", "drivers/net/driver.c", 100, "definition"),
                "func_b": FunctionCall("func_b", "fs/ext4/inode.c", 200, "definition"),
                "func_c": FunctionCall("func_c", "kernel/sched/core.c", 300, "definition"),
        ...
        assert self.tracer._should_exclude_path(["func_c", "func_a"], exclude_directories=exclude_directories) is True
        assert self.tracer._should_exclude_path(["func_a"], exclude_directories=exclude_directories) is True

        # Path not containing functions from drivers should not be excluded
        assert not self.tracer._should_exclude_path(["func_c", "func_d"], exclude_directories=exclude_directories)
        ...
        assert self.tracer._should_exclude_path(["func_c", "func_a"], exclude_directories=exclude_directories) is True
        assert self.tracer._should_exclude_path(["func_c", "func_b"], exclude_directories=exclude_directories) is True

        # Path not containing functions from excluded directories should not be excluded
        assert not self.tracer._should_exclude_path(["func_c", "func_d"], exclude_directories=exclude_directories)

        # Test with no excluded directories
        assert not self.tracer._should_exclude_path(["func_a", "func_b"], exclude_directories=None)
        assert not self.tracer._should_exclude_path(["func_a", "func_b"], exclude_directories=set())

    def test_should_exclude_path_combined_filters(self):
        ...
        assert self.tracer._should_exclude_path(["func_c", "func_b"], exclude_functions, exclude_directories) is True

        # Path should be excluded if it contains function from excluded directory
        assert self.tracer._should_exclude_path(["func_c", "func_a"], exclude_functions, exclude_directories) is True

        # Path should not be excluded if it contains neither
        assert not self.tracer._should_exclude_path(["func_c"], exclude_functions, exclude_directories)

    def test_trace_callers_with_exclude_directories(self):
        """Test caller tracing with directory exclusions."""
        # Mock function definitions for directory testing
        def mock_find_function_definition(function):
            file_map = {
                "caller1": FunctionCall("caller1", "drivers/net/driver.c", 100, "definition"),
                "caller2": FunctionCall("caller2", "kernel/sched/core.c", 200, "definition"),
                "target_func": FunctionCall("target_func", "mm/page_alloc.c", 300, "definition"),
            }
            return file_map.get(function)
        ...
        # Mock callers
        def mock_get_functions_calling(function):
            if function == "target_func":
                return [
        ...
        # Test excluding drivers directory
        exclude_directories = {"drivers"}
        paths = self.tracer.trace_callers("target_func", exclude_directories=exclude_directories)
    ...
    def test_trace_callees_with_exclude_directories(self):
        """Test callee tracing with directory exclusions."""
        # Mock function definitions for directory testing
        def mock_find_function_definition(function):
            file_map = {
                "source_func": FunctionCall("source_func", "kernel/sched/core.c", 100, "definition"),
                "callee1": FunctionCall("callee1", "drivers/net/driver.c", 200, "definition"),
                "callee2": FunctionCall("callee2", "mm/page_alloc.c", 300, "definition"),
            }
            return file_map.get(function)
        ...
        # Test excluding drivers directory
        exclude_directories = {"drivers"}
        paths = self.tracer.trace_callees("source_func", exclude_directories=exclude_directories)
    ...
    def test_get_unique_call_chains_with_exclude_directories(self):
        ...
        # Test excluding drivers directory
        exclude_directories = {"drivers"}
        paths = self.tracer.get_unique_call_chains("target_func", exclude_directories=exclude_directories)
    ...
    def test_trace_callers_with_callback_search_enabled(self):
        """Test caller tracing with callback search enabled."""
        # Mock cscope to use callback search
        def mock_get_callback_callers(function):
            if function == "target_func":
                return [FunctionCall("callback_caller", "file1.c", 10, "ops->target_func()")]
            else:
                return []
        ...
        tracer = CallTracer(self.mock_cscope, max_depth=3, enable_callback_search=True)
        paths = tracer.trace_callers("target_func")
    ...
    def test_trace_callers_with_callback_search_disabled(self):
