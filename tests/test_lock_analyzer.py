...
class TestLockAnalyzer:
    ...
    def test_find_lock_operations(self):
        ...
        operations = self.analyzer.find_lock_operations("test_func")
    ...
    def test_analyze_path_locks(self):
        ...
        self.analyzer.find_lock_operations = mock_find_operations
        ...
        self.mock_cscope.get_functions_called_by = mock_get_called_by

        context = self.analyzer._analyze_path_locks(call_path)
    ...
    def test_check_lock_protection(self):
        ...
        # Mock lock contexts
        def mock_analyze_context(func, locks, unique_only=True, exclude_functions=None, exclude_directories=None):
            if locks == ["test_lock"]:
                return [
        ...
        results = self.analyzer.check_lock_protection("target", "test_lock")

        assert len(results) == 2
        assert results["caller1 -> target"] is True
        assert results["caller2 -> target"] is False

    def test_find_unprotected_calls(self):
        ...
        def mock_analyze_context(func, locks=None, unique_only=True, exclude_functions=None, exclude_directories=None):
            return mock_contexts

        self.analyzer.analyze_lock_context = mock_analyze_context

        unprotected = self.analyzer.find_unprotected_calls("target", ["lock1", "lock2"])
    ...
    def test_get_lock_summary(self):
        ...
        def mock_analyze_context(func, locks=None, unique_only=True, exclude_functions=None):
            return mock_contexts

        self.analyzer.analyze_lock_context = mock_analyze_context

        summary = self.analyzer.get_lock_summary("target")
    ...
    def test_rcu_lock_detection(self):
        ...
        # Test RCU lock matching (generic targets)
        assert self.analyzer._lock_matches_target("rcu_read_lock", ["rcu_var"]) is True
        assert self.analyzer._lock_matches_target("rcu_read_lock", ["rcu"]) is True
        assert (
            self.analyzer._lock_matches_target("rcu_read_unlock", ["rcu_var"]) is True
        )
    ...
    def test_rcu_lock_context_analysis(self):
    ...
    def test_call_order_filtering(self):
        ...
        context = self.analyzer._analyze_path_locks(mock_path)
    ...
    def test_lock_operations_display_includes_releases(self):
    ...
    def test_function_name_in_lock_operations(self):
        ...
        operations = self.analyzer.find_lock_operations("test_function")
    ...
    def test_identify_rtnl_lock_operations(self):
        ...
        call = FunctionCall("rtnl_net_lock", "net/core/rtnetlink.c", 30, "rtnl_net_lock(&rtnl_net_lock);")
        operations = self.analyzer._identify_lock_operation(call, "test_func")
...
class TestLockAnalyzerExcludeFunctions:
    ...
    def test_analyze_lock_context_with_exclude_functions(self):
        ...
        contexts = self.analyzer.analyze_lock_context("target", unique_only=True, exclude_functions=exclude_functions)

        # Verify that tracer was called with exclude_functions
        self.mock_tracer.get_unique_call_chains.assert_called_once_with("target", exclude_functions=exclude_functions, exclude_directories=None)

        # Should return the filtered contexts
        assert isinstance(contexts, list)

    def test_check_lock_protection_with_exclude_functions(self):
        ...
        self.analyzer.analyze_lock_context = Mock(return_value=[mock_context])

        exclude_functions = {"excluded_func"}
        results = self.analyzer.check_lock_protection("target", "test_lock", unique_only=True, exclude_functions=exclude_functions)

        # Verify that analyze_lock_context was called with exclude_functions
        self.analyzer.analyze_lock_context.assert_called_once_with("target", ["test_lock"], True, exclude_functions, None)

        # Should return filtered results
        assert isinstance(results, dict)

    def test_find_unprotected_calls_with_exclude_functions(self):
        ...
        exclude_functions = {"excluded_func"}
        results = self.analyzer.find_unprotected_calls("target", ["test_lock"], unique_only=True, exclude_functions=exclude_functions)
    ...
    def test_analyze_lock_context_with_exclude_directories(self):
        ...
        results = self.analyzer.analyze_lock_context("target_func", exclude_directories=exclude_directories)
        ...
        self.mock_tracer.get_unique_call_chains.assert_called_once_with(
            "target_func", exclude_functions=None, exclude_directories=exclude_directories
        )

        assert isinstance(results, list)

    def test_check_lock_protection_with_exclude_directories(self):
        ...
        exclude_directories = {"drivers"}
        results = self.analyzer.check_lock_protection("target_func", "my_lock", exclude_directories=exclude_directories)
    ...
    def test_find_unprotected_calls_with_exclude_directories(self):
        ...
        results = self.analyzer.find_unprotected_calls("target_func", required_locks, exclude_directories=exclude_directories)
